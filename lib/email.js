...
function generateICS({ title, description, location, startDate, endDate, organizerName, organizerEmail }) {
  const formatDate = (d) => {
  ...
  return [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Portfolio Meeting System//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:REQUEST',
    'BEGIN:VEVENT',
    `UID:meeting-${Date.now()}@portfolio`,
